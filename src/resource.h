	...
	struct unsigned_color
	{
		...
		static unsigned_color from_color(const color& color)
		{
			return unsigned_color{
				std::clamp(static_cast<uint8_t>(color.r * 200.f), kMin, kMax),
			  	std::clamp(static_cast<uint8_t>(color.g * 200.f), kMin, kMax),
			  	std::clamp(static_cast<uint8_t>(color.b * 200.f), kMin, kMax)
			};
		};
		static unsigned_color from_float3(const float3& color)
		{
			auto c = clamp(color * 200., 0., 200.);
			return unsigned_color{
				static_cast<uint8_t>(c.x),
				static_cast<uint8_t>(c.y),
				static_cast<uint8_t>(c.z)
			};
		};
		float3 to_float3() const
		{
